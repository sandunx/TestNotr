        ...
        try {
            if (isset($_POST) && !empty($_POST)) {
                $keys = array(
                    ...
                    'debugInfo', 'searchCF', 'searchCFLabel', 'newProductQty', 'newProductStatus', 'directDbUpdate', 'directDbSearch', 'customCss', 'customCssMobile',
                    'show_price_1', 'show_price_2', 'show_price_3', 'price_1_label', 'price_2_label', 'price_3_label', 'cartDecimalQuantity',
                    'defaultPriceField', 'sendAdminEmailCreatedOrder', 'sendClientEmailCreatedOrder', 'price_1_field', 'price_2_field', 'price_3_field', 'productStatuses',
                    'orderStatuses', 'productsCatalogVisibility', 'displayPayButton',
                    ...
                    'notifyUsersStock', 'nowOrderDefaultUser', 'displaySearchCounter', 'openOrderAfterCreation', 'shippingRequired', 'paymentRequired', 'orderFulfillmentEnabled',
                    'orderFulFillmentField', 'receipt-width', 'receipt-template', 'modifyPreProcessSearchString', 'receiptOrderPreview', 'autoStatusFulfilled',
                    'delayBetweenScanning', 'displayCouponField', 'displayNoteField', 'defaultShippingMethods', 'orderFulfillmentByDefault',
                    'productLeftSidebarWidth', 'productMiddleRightWidth', 'productMiddleLeftWidth'
                );
                foreach ($keys as $key) {
                    if (isset($_POST[$key])) {
                        $this->post[$key] = $this->getRequestData($_POST[$key], $key);

                        if (in_array($key, array("productStatuses", "orderStatuses", 'productsCatalogVisibility', "notifyUsersStock", 'defaultShippingMethods')) && is_array($this->post[$key])) {
                ...
                if (isset($this->post["key"]) || isset($this->post["m_key"])) {
                    $prefix = "";
                    @delete_transient($prefix . 'ukrsolution_upgrade_scanner_1.5.3');
            ...
            if (!$tab) {
                ...
                foreach ($settingsTable as $key => $setting) {
                    if ($setting->key === "settings_prices") {
                        ...
                        'openOrderAfterCreation', 'shippingRequired', 'paymentRequired', "fieldForNewProduct", 'orderFulfillmentEnabled', 'orderFulFillmentField',
                        'receipt-width', 'receipt-template', 'modifyPreProcessSearchString', 'updated_timestamp', 'autoStatusFulfilled', 'delayBetweenScanning',
                        'displayCouponField', 'displayNoteField', 'displayPayButton', 'orderFulfillmentByDefault',
                        'productLeftSidebarWidth', 'productMiddleRightWidth', 'productMiddleLeftWidth'
                    ))) {
                        $settings[$setting->key] = $setting->value;
                    } else if (in_array($setting->key, ["defaultOrderStatus", "defaultShippingMethod", "defaultPriceField", "allowNegativeStock", 'defaultShippingMethods'])) {
                        $settings[$setting->key] = $setting->value;
                    }
                }
