...
class Results
{
    ...
    public function getShopProductTax($price, $productTaxClass)
    {
        $tax = 0;
        ...
        try {
            ...
            $tax_rates_data = $this->getShopTaxRate($productTaxClass);
    ...
    public function getShopShippingPriceTax($price)
    {
        $tax = 0;

        try {
            $tax_rates_data = $this->getShopTaxRate('');

            $tax_amounts = \WC_Tax::calc_shipping_tax($price, $tax_rates_data, $price);
            $tax = array_sum($tax_amounts);
        } catch (\Throwable $th) {
        }
