    ...
    public function check($token, $userToken = "", $userSession = "")
    {
        ...
        if ($userToken) {
            ...
            $userMeta = $wpdb->get_row(
                $wpdb->prepare("SELECT UM.meta_key, UM.meta_value, UM.user_id FROM {$wpdb->usermeta} AS UM WHERE UM.meta_key IN ('barcode_scanner_app_otp') AND UM.meta_value = '%s';", $userToken)
            );
            ...
            $userMeta = $wpdb->get_row(
                $wpdb->prepare("SELECT UM.meta_key, UM.meta_value, UM.user_id FROM {$wpdb->usermeta} AS UM WHERE UM.meta_key IN ('barcode_scanner_app_otp') AND UM.meta_value = '%s';", $token)
            );
