...
class CartScannerActions
{
    ...
    private function getCartDetails(WP_REST_Request $request = null)
    {
        ...
        if ($activeShippingMethod) {
            ...
            if ($shipping && isset($shipping["cost"])) {
                ...
                if ($customerUserId) {
                    if ($taxBasedOn == 'base') {
                        $cartShippingTax = (new Results)->getShopShippingPriceTax($cartShippingTotal);
                    } else {
        ...
        foreach ($items as $item) {
            ...
            if ($product) {
                if (($customerUserId || $taxBasedOn == 'base') && $product->get_tax_status() == 'taxable') {
                    ...
                    if ($taxBasedOn == 'base') {
                        $line_subtotal_tax = (new Results)->getShopProductTax($priceForTaxes, $product->get_tax_class());
                    } else {
            ...
            if ($taxBasedOn == 'base') {
                $_rate =  (new Results)->getShopTaxRate($product->get_tax_class());
            } else {
    ...
    public function orderCreate(WP_REST_Request $request)
    {
        ...
        if ($order) {
            ...
            if ($taxBasedOn == 'base') {
                $taxDetails = (new Results)->getShopTaxRate('');
            } else {
