...
class ManagementActions
{
    ...
    public function productSearch(WP_REST_Request $request, $actions = true, $findById = false, $actionError = "")
    {
        ...
        if ($products) {
            ...
            $requestName = $request->get_param("requestbs");
    ...
    private function applyFulfillment(WP_REST_Request $request, $orderId, $product)
    {
        ...
        foreach ($order->get_items() as $itemId => $value) {
            ...
            if ($pid == $product["ID"] && ($value->get_variation_id() == $product["variation_id"] || $value->get_variation_id() == 0)) {
                $result = $this->orderUpdateItemMeta($request, $orderId, $itemId, array(
                    array("key" => "usbs_check_product", "value" => time())
                ), $product);
            }
        }

        $this->checkFulfillmentOrderData($orderId, $result, $request);

        return $result;
    }

    public function checkFulfillmentOrderData($orderId, &$result = null, $request = null, $isCheckStatus = true)
    {
        $orderRequest = new WP_REST_Request("", "");
        $orderRequest->set_param("query", $orderId);
        $orderRequest->set_param("filterExcludes", array("products"));
        $orders = $this->orderSearch($orderRequest, false, true);

        if ($orders->data && isset($orders->data["orders"]) && count($orders->data["orders"]) == 1) {
            $infoData = $this->isOrderFulfillment($orders->data["orders"][0]);

            if ($infoData) {
                update_post_meta($orderId, "usbs_order_fulfillment_data", $infoData);

                if ($isCheckStatus && $infoData["totalScanned"] && $infoData["totalQty"] == $infoData["totalScanned"]) {
                    $this->updateOrderStatusByFulFillment($orders->data["orders"][0], $result, $request);
                }
            }
    ...
    private function updateOrderStatusByFulFillment($orderData, &$result = null, $request = null)
    {
        try {
    ...
    public function orderUpdateItemMeta(WP_REST_Request $request, $orderId = null, $itemId = null, $fields = null, $product = null)
    {
        ...
        $empty = array();
        $checker = $this->checkFulfillmentOrderData($orderId, $empty, $request);
    ...
    public function updateOrderFulfilledObject(WP_REST_Request $request)
    {
        ...
        $empty = null;
        $this->checkFulfillmentOrderData($id, $empty, $request);
    ...
    public function removeFulfillmentObject(WP_REST_Request $request)
    {
        ...
        $empty = array();
        $this->checkFulfillmentOrderData($id, $empty, null, false);
