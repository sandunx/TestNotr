...
class Database
{
    ...
    public static function updatePost($id, $additionalColumns = array(), $post = null, $indexedRecord = null, $trigger = "")
    {
        ...
        if ($post && in_array($post->post_type, array("product", "product_variation", "shop_order"))) {
            ...
            $data = array(
                ...
                "{$prefix}_ywbc_barcode_display_value" => get_post_meta($id, "_ywbc_barcode_display_value", true),
        ...
        Debug::addPoint("UpdatePosts->check-fulfillment id" . $id);
    ...
    private static function checkOrderFulfillment($orderId)
    {
        try {
            ...
            $empty = null;
            $infoData = self::$managementActions->checkFulfillmentOrderData($orderId, $empty, null, false);
